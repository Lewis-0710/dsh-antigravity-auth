/**
 * Regression guard for the P1 review on the trailing-empty-frame fix.
 *
 * A signed tool call followed by a signature-bearing EMPTY text frame used to
 * emit a `block-start` that was never closed: the stream grammar forbids an
 * open block at `finish`, and DSH's `BlockAssembler` drops the WHOLE turn's
 * replay envelope when its entries no longer align with the emitted blocks,
 * which silently loses the tool call's replay signature.
 *
 * These cases therefore assert through the real `BlockAssembler` — not by
 * counting `block-end` chunks — that the emitted blocks and the replay
...
/** Drive the adapter through the canonical DSH assembler the agent loop uses. */
...
describe('Antigravity stream and replay alignment', () => {
  it('keeps replay metadata when a signed empty text frame trails a signed tool call', async () => {
    ...
    // The content-free frame must not become a block of its own.
    ...
    // The provider-issued signature must survive into the replay envelope.
