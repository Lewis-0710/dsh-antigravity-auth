  ...
  return async ({ code, verifier, signal }) => {
    ...
    const email = typeof payload.email === 'string' && payload.email.includes('@')
      ...
      : typeof payload.id_token === 'string'
        ...
        : await fetchUserInfoEmail(fetchImpl, payload.access_token, signal)
    return {
...
function isAllowedCallbackHost(value: string): boolean {
  return ANTIGRAVITY_CALLBACK_HOSTS.includes(value.toLowerCase() as (typeof ANTIGRAVITY_CALLBACK_HOSTS)[number])
}
